...
const ArrowsProgress = ({
  arrowProps,
  current,
  max,
  ...progressNumberSettings
}) => {
  return (
    <div className={styles.arrowsProgress}>
      <Arrow {...arrowProps.prev} />
      <div className={styles.progress}>
        <Typography as={"span"} {...progressNumberSettings}>
...
const ModalSlider = ({ data, isOpen, settings, initalIndex = 0, onClose }) => {
  ...
  const handleRealIndexChange = (swiper) => setActiveIndex(swiper.realIndex);
  return (
    <Modal isOpen={isOpen} withCloseButton onClose={onClose}>
        ...
        >
          {data.map(({ photo, area, _id }, index) => (
            <SwiperSlide key={_id ?? index}>
              <div>
                <Typography
                  {...settings.modalSlider.area}
                  className={styles.modalSliderWrapper}
                >
                  {area}
                </Typography>
                <img src={photo} alt="" />
        ...
        <ul className={styles.roomCharacteristics}>
          {characteristics.map(({ title, value, _id }, index) => (
            <li className={styles.roomCharacteristic} key={_id ?? index}>
              <Typography as={"h5"} {...blockSettings.characteristic.title}>
                {title}
              </Typography>
              <Typography as={"span"} {...blockSettings.characteristic.value}>
                {value}
              </Typography>
            </li>
          ))}
        ...
        <ui className={styles.roomAdvantages}>
          {advantages.map(({ icon, title, _id }, index) => (
            <li key={_id ?? index}>
              ...
              <Typography as={"span"} {...blockSettings.advantageTitle}>
                {title}
              </Typography>
            </li>
          ))}
